...
async def main():
    async with httpx.AsyncClient(timeout=10.0) as client:
        ...
        r = await client.get("https://lrclib.net/api/get", params={
        ...
        r = await client.get("https://lrclib.net/api/search", params={
            "track_name": "Mr. Brightside",
            "artist_name": "The Killers",
        })
        print(f"Status: {r.status_code}")
        ...
        for entry in results[:3]:
            print(f"  [{entry.get('id')}] {entry.get('artistName')} — {entry.get('trackName')} (has lyrics: {bool(entry.get('plainLyrics') or entry.get('syncedLyrics'))})")
